          ...
          <ul>
            <li v-for="task in tasks" :key="task.id">
              ...
              <label class="flex items-center h-10 px-2 rounded cursor-pointer hover:bg-gray-900" :for="'task_' + task.id">
                ...
                <span v-if="!task.checked && !editingTask" class="ml-4 text-sm" :class="{ 'line-through': task.checked }">{{ task.text }}</span>
              </label>
              <div class="flex justify-end">
                <button v-if="!task.checked && !editingTask" class="text-info" data-mdb-toggle="tooltip" title="Edit todo" @click="editTask(task)">
                ...
                <button class="text-danger" data-mdb-toggle="tooltip" title="Delete todo" @click="removeTask(task)">
